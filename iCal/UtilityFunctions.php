 ...
 *
 */
class iCal_UtilityFunctions {
  // Store the single instance of iCal_UtilityFunctions
  ...
  public static function getInstance() {
    if (!self::$m_pInstance)
      self::$m_pInstance = new iCal_UtilityFunctions();
  ...
  public static function _chkdatecfg( $theDate, & $parno, & $params ) {
    ...
    elseif( isset( $params['VALUE'] ) && ( 'DATE' == $params['VALUE'] ))
      ...
      if( is_array( $theDate )) {
        ...
        if( !empty( $tzid )) {
          $parno = 7;
          if( !iCal_UtilityFunctions::_isOffset( $tzid ))
        ...
        elseif((( 8 == strlen( $date ) && ctype_digit( $date )) || ( 11 >= strlen( $date ))) &&
          ...
          $parno = 3; // DATE
        $date = iCal_UtilityFunctions::_date_time_string( $date, $parno );
        if( !empty( $date['tz'] )) {
          $parno = 7;
          if( !iCal_UtilityFunctions::_isOffset( $date['tz'] ))
  ...
  public static function createTimezone( & $calendar, $timezone, $xProp=array() ) {
    ...
    foreach( $transitions as $trans ) {
      ...
      if( TRUE !== $trans['isdst'] ) {
        ...
        $stdTZOFFSETTO = $dlghtTZOFFSETFROM = iCal_UtilityFunctions::offsetSec2His( $trans['offset'] );
        $stdTZNAME     = $trans['abbr'];
      }
      else {
        $dlghtDTSTART    = $trans['time'];
        $dlghtTZOFFSETTO = $stdTZOFFSETFROM = iCal_UtilityFunctions::offsetSec2His( $trans['offset'] );
    ...
    if(( $stdTZOFFSETTO != $stdTZOFFSETFROM  ) && ( FALSE === iCal_UtilityFunctions::_setTZrrule( $std )))
    ...
    if( FALSE === iCal_UtilityFunctions::_setTZrrule( $dlght ))
 ...
 */
  public static function _date2timestamp( $datetime, $tz=null ) {
    ...
    if( $tz )
      $datetime['tz'] = $tz;
    $offset = ( isset( $datetime['tz'] ) && ( '' < trim ( $datetime['tz'] ))) ? iCal_UtilityFunctions::_tz2offset( $datetime['tz'] ) : 0;
 ...
 */
  public static function _duration_string( $duration ) {
    ...
    return iCal_UtilityFunctions::_duration_array( $output );
  }
/**
 ...
 */
  public static function _isArrayDate( $input ) {
    ...
    if(( isset( $input[0] ) && isset( $input[1] ) && isset( $input[2] )) &&
      ...
      return TRUE;
    $input = iCal_UtilityFunctions::_date_time_string( $input[1].'/'.$input[2].'/'.$input[0], 3 ); //  m - d - Y
    if( isset( $input['year'] ) && isset( $input['month'] ) && isset( $input['day'] ))
      return checkdate( (int) $input['month'], (int) $input['day'], (int) $input['year'] );
    return FALSE;
  }
...
// echo "recur __in_ comp start ".implode('-',$wdate)." period start ".implode('-',$startdate)." period end ".implode('-',$enddate)."<br />\n";print_r($recur);echo "<br />\n";//test###
    $endDatets = iCal_UtilityFunctions::_date2timestamp( $enddate ); // fix break
...
// echo "bysetposXold_start=$bysetposYold $bysetposMold $bysetposDold<br />\n"; // test ###
      ...
      if( 'YEARLY' == $recur['FREQ'] ) {
        ...
        $wdatets        = iCal_UtilityFunctions::_date2timestamp( $wdate );
        iCal_UtilityFunctions::_stepdate( $enddate, $endDatets, array( 'year' => 1 )); // make sure to count whole last year
      }
      elseif( 'MONTHLY' == $recur['FREQ'] ) {
        $wdate['day']   = 1; // start from beginning of month
        $wdatets        = iCal_UtilityFunctions::_date2timestamp( $wdate );
        iCal_UtilityFunctions::_stepdate( $enddate, $endDatets, array( 'month' => 1 )); // make sure to count whole last month
      }
      else
        iCal_UtilityFunctions::_stepdate( $enddate, $endDatets, $step); // make sure to count whole last period
...
// echo "cont: ".implode('-',$wdate)." ix=$intervalix old=$currentKey interval=".$intervalarr[$intervalix]."<br />\n";//test
      ...
      if( $updateOK && isset( $recur['BYMONTH'] ))
        $updateOK = iCal_UtilityFunctions::_recurBYcntcheck( $recur['BYMONTH']
                                           , $wdate['month']
                                           ,($wdate['month'] - 13));
      ...
      if( $updateOK && isset( $recur['BYYEARDAY'] ))
        $updateOK = iCal_UtilityFunctions::_recurBYcntcheck( $recur['BYYEARDAY']
                                           , $daycnts[$wdate['month']][$wdate['day']]['yearcnt_up']
                                           , $daycnts[$wdate['month']][$wdate['day']]['yearcnt_down'] );
      if( $updateOK && isset( $recur['BYMONTHDAY'] ))
        $updateOK = iCal_UtilityFunctions::_recurBYcntcheck( $recur['BYMONTHDAY']
                                           , $wdate['day']
        ...
        if( isset( $recur['BYDAY']['DAY'] )) { // single day, opt with year/month day order no
          ...
          if( isset( $recur['BYDAY'][0] )) {
            ...
            if(( isset( $recur['FREQ'] ) && ( $recur['FREQ'] == 'MONTHLY' )) || isset( $recur['BYMONTH'] ))
              $daynosw = iCal_UtilityFunctions::_recurBYcntcheck( $recur['BYDAY'][0]
          ...
          foreach( $recur['BYDAY'] as $bydayvalue ) {
            ...
            if( isset( $bydayvalue[0] )) {
              ...
              if(( isset( $recur['FREQ'] ) && ( $recur['FREQ'] == 'MONTHLY' )) ||
                   isset( $recur['BYMONTH'] ))
                $daynosw = iCal_UtilityFunctions::_recurBYcntcheck( $bydayvalue['0']
                                                  , $daycnts[$m][$d]['monthdayno_up']
                                                  , $daycnts[$m][$d]['monthdayno_down'] );
              elseif( isset( $recur['FREQ'] ) && ( $recur['FREQ'] == 'YEARLY' ))
                $daynosw = iCal_UtilityFunctions::_recurBYcntcheck( $bydayvalue['0']
  ...
  public static function _recurIntervalIx( $freq, $date, $wkst ) {
    ...
    switch( $freq ) {
      ...
      case 'MONTHLY':
        ...
        break;
      case 'WEEKLY':
        $wdatets    = iCal_UtilityFunctions::_date2timestamp( $date );
  ...
  public static function _setRexrule( $rexrule ) {
    ...
    foreach( $rexrule as $rexrulelabel => $rexrulevalue ) {
      ...
      if( 'UNTIL'  != $rexrulelabel )
        ...
        if( iCal_UtilityFunctions::_isArrayTimestampDate( $rexrulevalue )) // timestamp date
          $input[$rexrulelabel] = iCal_UtilityFunctions::_timestamp2date( $rexrulevalue, 6 );
        elseif( iCal_UtilityFunctions::_isArrayDate( $rexrulevalue )) // date-time
          $input[$rexrulelabel] = iCal_UtilityFunctions::_date_time_array( $rexrulevalue, 6 );
        elseif( 8 <= strlen( trim( $rexrulevalue ))) // ex. 2006-08-03 10:12:18
          $input[$rexrulelabel] = iCal_UtilityFunctions::_date_time_string( $rexrulevalue );
 ...
 */
  public static function _setDate( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $params=FALSE, $caller=null, $objName=null, $tzid=FALSE ) {
    ...
    if( iCal_UtilityFunctions::_isArrayDate( $year )) {
      ...
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE-TIME', $hitval );
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE', 3, count( $year ), $parno );
      $input['value']  = iCal_UtilityFunctions::_date_time_array( $year, $parno );
    }
    elseif( iCal_UtilityFunctions::_isArrayTimestampDate( $year )) {
      ...
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE', 3 );
      $hitval          = ( isset( $year['tz'] )) ? 7 : 6;
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE-TIME', $hitval, $parno );
      $input['value']  = iCal_UtilityFunctions::_timestamp2date( $year, $parno );
    }
    elseif( 8 <= strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      ...
      elseif( $tzid && iCal_UtilityFunctions::_isOffset( substr( $year, -7 ))) {
      ...
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE-TIME', 7, $parno );
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE', 3, $parno, $parno );
      $input['value']  = iCal_UtilityFunctions::_date_time_string( $year, $parno );
    }
    else {
      if( is_array( $params )) {
      ...
      elseif( is_array( $tz )) {
        $input['params'] = iCal_UtilityFunctions::_setParams( $tz,     array( 'VALUE' => 'DATE-TIME' ));
        $tz = FALSE;
      }
      elseif( is_array( $hour )) {
        $input['params'] = iCal_UtilityFunctions::_setParams( $hour,   array( 'VALUE' => 'DATE-TIME' ));
      ...
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE', 3 );
      $hitval          = ( !empty( $tz )) ? 7 : 6;
      $parno           = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE-TIME', $hitval, $parno, $parno );
    ...
    if( !empty( $input['value']['tz'] ) && ( 'Z' != $input['value']['tz'] ) &&
      ( !iCal_UtilityFunctions::_isOffset( $input['value']['tz'] )))
 ...
 */
  public static function _setDate2( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $params=FALSE ) {
    ...
    $parno = iCal_UtilityFunctions::_existRem( $input['params'], 'VALUE', 'DATE-TIME', 7 ); // remove default
 ...
 */
  public static function _setMval( & $valArr, $value, $params=FALSE, $defaults=FALSE, $index=FALSE ) {
    ...
    elseif( 0 < count( $valArr )) {
      ...
      $index = 0;
    $valArr[$index] = array( 'value' => $value, 'params' => iCal_UtilityFunctions::_setParams( $params, $defaults ));
    ksort( $valArr );
  }
/**
