 ...
 */
require_once 'Scss/Type.php';
require_once 'Scss/Util.php';
require_once 'Scss/Block.php';
require_once 'Scss/Parser.php';
require_once 'Scss/Colors.php';
require_once 'Scss/Compiler/Environment.php';
require_once 'Scss/Node/Number.php';
require_once 'Scss/Formatter/OutputBlock.php';
...
class HTML_Scss
{
    ...
    public function compile($code, $path = null)
    {
        ...
        $formatter = 'HTML_Scss_Formatter_' . $this->formatter;
        $fn = str_replace('_', '/', $formatter ) . '.php';
        require_once $fn;
        ...
        if ($this->sourceMap) {
            if (is_object($this->sourceMap) && $this->sourceMap instanceof HTML_Scss_SourceMap_SourceMapGenerator) {
            	...
            	require_once 'Scss/SourceMap/SourceMapGenerator.php';
     ...
     */
    protected function compileMedia(HTML_Scss_Block $media)
    {
        $this->pushEnv($media);
     ...
     * @return \Leafo\ScssPhp\Formatter\OutputBlock
     */
    protected function mediaParent(HTML_Scss_Formatter_OutputBlock $scope)
    {
     ...
     */
    protected function compileDirective(HTML_Scss_Block $block)
    {
        $s = '@' . $block->name;
     ...
     * @param \Leafo\ScssPhp\Block $block
     */
    protected function compileAtRoot(HTML_Scss_Block $block)
    {
     ...
     * @return boolean
     */
    private function isWithout($without, HTML_Scss_Block $block)
    {
     ...
     * @param array                $selectors
     */
    protected function compileKeyframeBlock(HTML_Scss_Block $block, $selectors)
    {
        $env = $this->pushEnv($block);

        $envs = $this->compactEnv($env);

        $this->env = $this->extractEnv(array_filter($envs, function (HTML_Scss_Compiler_Environment $e) {
     ...
     * @param array                $selectors
     */
    protected function compileNestedBlock(HTML_Scss_Block $block, $selectors)
    {
        $this->pushEnv($block);
     ...
     * @param \Leafo\ScssPhp\Block $block
     */
    protected function compileBlock(HTML_Scss_Block $block)
    {
        $env = $this->pushEnv($block);
    ...
    protected function compileImport($rawPath, $out, $once = false)
    {
        ...
        if ($rawPath[0] === HTML_Scss_Type::T_STRING) {
            ...
            //var_dump($path);
            if ($npath = $this->findImport($path)) {
                if (! $once || ! in_array($npath, $this->importedFiles)) {
                    $this->importFile($npath, $out);
                    $this->importedFiles[] = $npath;
                }

                return true;
            }
            die("could not find @import : " . $path ."\n");
     ...
     * @return array
     */
    protected function compileChild($child, HTML_Scss_Formatter_OutputBlock $out)
    {
        ...
        switch ($child[0]) {
            ...
            case HTML_Scss_Type::T_IMPORT:
                list(, $rawPath) = $child;
                //echo "import:"; print_r($child);
                $rawPath = $this->reduce($rawPath);
                //echo "rawpath:"; print_r($rawPath);
    ...
    {
        //echo "compileValue: "; print_r($value);
        ...
        switch ($type) {
            ...
            case HTML_Scss_Type::T_COLOR:
                ...
                if (count($value) === 5 && $value[4] !== 1) { // rgba
                    $a = new HTML_Scss_Node_Number($value[4], '');
    ...
    {
        $parts = [];
        //echo "compileStringContent: "; print_r($string);
    ...
    public function throwError($msg)
    {
        ...
        $line = $this->sourceLine;
        $msg = "$msg: line: $line";
	require_once 'Scss/Exception/CompilerException.php';
    ...
    protected function coerceColor($value)
    {
        switch ($value[0]) {
            ...
            case HTML_Scss_Type::T_KEYWORD:
                ...
                if (isset(HTML_Scss_Colors::$cssColors[$name])) {
                    $rgba = explode(',', HTML_Scss_Colors::$cssColors[$name]);
    ...
    {
        $color = $this->assertColor($args[0]);
        require_once 'Scss/Base/Range.php';
        $amount = HTML_Scss_Util::checkRange('amount', new HTML_Scss_Base_Range(0, 100), $args[1], '%');
    ...
    protected function libStrIndex($args)
    {
        ...
        return $result === false ? static::$null : new HTML_Scss_Node_Number($result + 1, '');
    }
