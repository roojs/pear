...
class HTML_Less_Tree_Mixin_Definition extends HTML_Less_Tree_Ruleset {
    ...
    public function compileParams($env, $mixinFrames, $args = array(), &$evaldArguments = array()) {
        $frame = new HTML_Less_Tree_Ruleset(null, array());
        $params = $this->params;
        $mixinEnv = null;
        $argsLength = 0;
                ...
                if ($arg && $arg['name']) {
                    ...
                    foreach ($params as $j => $param) {
                        if (!isset($evaldArguments[$j]) && $arg['name'] === $params[$j]['name']) {
                            ...
                            array_unshift($frame->rules, new HTML_Less_Tree_Rule($arg['name'], $arg['value']->compile($env)));
                            $isNamedFound = true;
                            break;
                        }
                    }
                    if ($isNamedFound) {
                        ...
                        throw new HTML_Less_Exception_Compiler("Named argument for " . $this->name . ' ' . $args[$i]['name'] . ' not found');
                    }
                }
            }
        }
        ...
        foreach ($params as $i => $param) {
            ...
            if (isset($param['name']) && $param['name']) {
                ...
                if (isset($param['variadic'])) {
                    ...
                    $expression = new HTML_Less_Tree_Expression($varargs);
                    array_unshift($frame->rules, new HTML_Less_Tree_Rule($param['name'], $expression->compile($env)));
                } else {
                    ...
                    if ($val) {
                        ...
                        if (!$mixinEnv) {
                            $mixinEnv = new HTML_Less_Environment();
                        ...
                        throw new HTML_Less_Exception_Compiler("Wrong number of arguments for " . $this->name . " (" . $argsLength . ' for ' . $this->arity . ")");
                    }

                    array_unshift($frame->rules, new HTML_Less_Tree_Rule($param['name'], $val));
    ...
    public function compile($env) {
        if ($this->frames) {
            return new HTML_Less_Tree_Mixin_Definition($this->name, $this->params, $this->rules, $this->condition, $this->variadic, $this->frames);
        }
        return new HTML_Less_Tree_Mixin_Definition($this->name, $this->params, $this->rules, $this->condition, $this->variadic, $env->frames);
    }
