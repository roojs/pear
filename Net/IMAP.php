 ...
 */
require_once 'Net/IMAP/Protocol.php';
 ...
 * @link     http://pear.php.net/package/Net_IMAP
 */
class Net_IMAP extends Net_IMAP_Protocol
{
    /**
     * Constructor
     *
     ...
     *        the IMAP AUTHENTICATE command but using the authMethod in 'string' )eg. OAUTH
    ...
    function getSummary($msg_id = null, $uidFetch = false)
    {
        ...
        if (isset($ret['PARSED'])) {
            for ($i=0; $i<count($ret['PARSED']); $i++) {
                ...
                if (isset($ret['PARSED'][$i]['COMMAND']) && $ret['PARSED'][$i]['COMMAND'] != 'FETCH') {
                    continue;
                }
