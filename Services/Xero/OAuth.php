...
require 'Services/Xero/OAuthSign.php';
    ...
    function diagnostics() 
    {
        ...
        if (empty($this->config ['application_type']) || $this->config ['application_type'] == 'Partner' || $this->config ['application_type'] == 'Private') {
            ...
            if (!file_exists($this->config ['rsa_public_key'])) {
                $testOutput ['rsa_cert_error'] = "Can't read the self-signed SSL cert. Private and Partner API applications require a self-signed X509 cert " .
            ...
            if (!file_exists($this->config ['rsa_private_key'])) {
                $testOutput ['rsa_cert_error'] = "Can't read the self-signed cert key. Check your rsa_private_key config variable. Private and Partner API applications require a self-signed X509 cert " .
                        "http://developer.xero.com/documentation/advanced-docs/public-private-keypair/ \n";
            }
