...
function __construct($orientation='P',$unit='mm',$format='A4')
{
    ...
    $this->CurPageFormat = $format; //FC with 1.6...
...
function Line($x1,$y1,$x2,$y2)
{
    ...
    if (!is_array($x1)) {
    	$this->_out(
            sprintf('%.2f %.2f m %.2f %.2f l S',$x1*$this->k,($this->h-$y1)*$this->k,$x2*$this->k,($this->h-$y2)*$this->k));
        return;
    }
    $type= 'm';$out='';
    for ($i=0;$i<count($x1);$i+=2) {
       	$out .= sprintf(' %.2f %.2f %s',$x1[$i]*$this->k,($this->h-$x1[$i+1])*$this->k,$type);
        $type = 'l';
    }    	
    $this->_out(ltrim($out) . ' f');
...
function AddFont($family,$style='',$file='')
{
    ...
    if (file_exists(dirname(__FILE__) .'/FpdfAlt/'.$file)) {
        include 'FpdfAlt/'.$file;
    }
	if(!isset($name)) {
...
function _parsepng($file)
{
	...
	elseif($ct==3)
		...
		$this->Error("Alpha channel not supported: $ct ".$file);
	if(ord(fread($f,1))!=0)
